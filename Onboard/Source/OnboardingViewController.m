...
- (instancetype)initWithContents:(NSArray *)contents {
    ...
    //self.pageControl.dotImage = [UIImage imageNamed:@"ac"];
  //  self.pageControl.selectedDotImage = [UIImage imageNamed:@"ac"];
   // [self.pageControl sizeToFit];
    ...
    [self.actionButton addTarget:self action:@selector(handleOneButtonPressed) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.secondactionButton = [UIButton new];
    
    
    self.secondactionButton.titleLabel.font = [UIFont fontWithName:kDefaultOnboardingFont size:kDefaultButtonFontSize];
    
    [self.secondactionButton setTitle:@"Continue" forState:UIControlStateNormal];
    
    [self.secondactionButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    
    [self.secondactionButton addTarget:self action:@selector(handleButtonPressed) forControlEvents:UIControlEventTouchUpInside];
    
    self.secondactionButton.backgroundColor = [UIColor blueColor];
    self.secondactionButton.layer.cornerRadius = 10;
    self.secondactionButton.clipsToBounds = YES;
    
    [self.secondactionButton addTarget:self action:@selector(handleTwoSkipButtonPressed) forControlEvents:UIControlEventTouchUpInside];
...
- (void)viewWillLayoutSubviews {
   ...
   // self.skipButton.frame = CGRectMake(CGRectGetMaxX(self.view.frame) - kSkipButtonWidth, CGRectGetMaxY(self.view.frame) - self.underPageControlPadding - kSkipButtonHeight, kSkipButtonWidth, kSkipButtonHeight);
    ...
    self.skipButton.frame = CGRectMake(CGRectGetMaxX(self.view.frame) - kSkipButtonWidth, 10.0, kSkipButtonWidth, kSkipButtonHeight);
    
    
    self.pageControl.frame = CGRectMake(0, CGRectGetMaxY(self.view.frame) - self.underPageControlPadding - kPageControlHeight-80, self.view.frame.size.width, kPageControlHeight);
    
    self.actionButton.frame = CGRectMake(10,CGRectGetMaxY(self.view.frame)-60, CGRectGetMaxX(self.view.frame)/2 - 15, 50);
    
    self.secondactionButton.frame = CGRectMake(CGRectGetMaxX(self.view.frame)/2 +5,CGRectGetMaxY(self.view.frame)-60, CGRectGetMaxX(self.view.frame)/2 - 15, 50);
...
- (void)generateView {
    ...
    [self.view addSubview:self.secondactionButton];
...
- (void)handleOneButtonPressed {
    if (self.btn1Handler) {
        self.btn1Handler();
    }
}

    - (void)handleTwoSkipButtonPressed {
        
        NSUInteger indexOfCurrentPage = [self.viewControllers indexOfObject:_currentPage];
        
        if(indexOfCurrentPage==2){
            [self.secondactionButton setTitle:@"Get Started" forState:UIControlStateNormal];
            if (self.btn2Handler) {
                self.btn2Handler();
            }
