...
- (void)updateViewConstraints {
    ...
    if (_simple) {
        ...
        NSDictionary *digits = @{@"d0": _digitLabels[0],
                                 @"d1": _digitLabels[1],
                                 @"d2": _digitLabels[2],
                                 @"d3": _digitLabels[3],
                                 @"d4": _digitLabels[4],
    ...
    [constraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-[promptLabel]-|" options:0 metrics:nil views:views]];
...
//                [_delegate PAPasscodeViewControllerDidEnterPasscode:self];
            ...
            [self validateDevice:_userID withcode:text];
